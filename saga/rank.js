...
function* getRankDate (rank_id){
	const result = yield _rankDateRequest.get({data: [{parent_id:rank_id}]});
...
function* getRecommendTag (classInfo){
	const {models, sortList} = yield _rankListRequest.get(classInfo);
...
function* listen_changeSongsOfRank() {
	while(true) {
		...
		yield put(push('Rank/'+rank_id+'/'+(pageIndex||1)))
	}
}
...
function* listen_changeSongsOfPage() {
	while(true) {
		...
		yield fork(getRankSongs, rank_id, pageIndex);
		yield put(push('Rank/'+rank_id+'/'+pageIndex));
	}
}
...
function* listen_initialContent (){
	const {activeTagId, pageIndex} = yield take(INITIAL_CONTENT);
	yield _rankListRequest.list.map((classInfo) => (
